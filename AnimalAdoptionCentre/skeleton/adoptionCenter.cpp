...
sortedAnimals{
    BST([](const Animal *a1, const Animal *a2)-> int {return a1->getSpecies().compare(a2->getSpecies());}),
    BST([](const Animal *a1, const Animal *a2)-> int {return a1->getAge() - a2->getAge();}),
    BST([](const Animal *a1, const Animal *a2)-> int {return a1->getHealthCondition().severity - a2->getHealthCondition().severity;}),
    BST([](const Animal *a1, const Animal *a2)-> int {return a1->getVaccinationStatus().getTotalHashValue() - a2->getVaccinationStatus().getTotalHashValue();}),
}{
}
...
void AdoptionCenter::addAnimal(Animal* a) {
    ...
    // TODO
    Animal **tem = new Animal*[numAnimals + 1];
    for(unsigned int i = 0; i < numAnimals; i++){
        tem[i] = animals[i];
    }
    tem[numAnimals] = a;
...
bool AdoptionCenter::removeAnimal(unsigned int id) {
    ...
    for(unsigned int i = 0; i < numAnimals; i++){
        ...
        sortedAnimals[NAME].remove(animals[i]);
        sortedAnimals[AGE].remove(animals[i]);
